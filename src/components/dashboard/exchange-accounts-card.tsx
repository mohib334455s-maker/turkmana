        ...
        {list.length === 0 ? (
          ...
          <ul className="divide-y divide-slate-100">
            {list.map((house) => {
              ...
              return (
                <li key={house.id} className="flex items-start justify-between gap-3 px-5 py-4">
                    ...
                    <div className="mt-2 space-y-1 text-[12px]">
                      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                        <span className="text-slate-500">
                          {tx('طلب بالای صرافی', 'Claim on exchange')}
                        </span>
                        <span className={cn('num font-semibold', claim ? 'text-emerald-700' : 'text-slate-400')}>
                          {formatCurrency(claim)}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                        <span className="text-slate-500">
                          {tx('باقیات از صرافی', 'Due from exchange')}
                        </span>
                        <span className={cn('num font-semibold', due ? 'text-rose-600' : 'text-slate-400')}>
                          {formatCurrency(due)}
                        </span>
                      </div>
                    </div>
